<template>
    <div>
        <h2
            class="text-3xl font-bold pb-4 inline-block tracking-wide border-b-4 mb-4 border-b-white pr-6"
        >
            Contact Details
        </h2>
        ...
        <form class="w-full max-w-lg" @submit.prevent="save">
            <div class="flex flex-wrap -mx-3 mb-6">
                <div class="w-1/2 mb-6 md:mb-0 pr-4">
                    <label
                        class="text-white w-full text-xs font-bold block py-4 tracking-wide"
                        >First Name</label
                    >
                    <input
                        v-model="first_name"
            ...
            <div class="mt-4">
                <label
                    class="text-white w-full text-xs font-bold block py-4 tracking-wide"
                    >Would you like to join our newsletter?</label
                >
                ...
                <div class="flex items-center">
                    ...
                    <HeadlessSwitch
                        ...
                        :class="
                            newsletterDisabled ? 'bg-slate-300' : 'bg-green-400'
                        "
                ...
                <label
                    class="text-white w-full text-xs font-bold block py-4 tracking-wide"
                    >Would you like us to send you a detailed quote?</label
                >
...
export default {
    ...
    computed: {
        ...
        visualiser() {
            ...
            return {
                engine: options.find(
                    (option) => option.id === visualiser.engine
                ),
                wheelbase: options.find(
                    (option) => option.id === visualiser.wheelbase
                ),
                horsepower: options.find(
                    (option) => option.id === visualiser.horsepower
                ),
                motion: options.find(
                    (option) => option.id === visualiser.motion
                ),
                transmission: options.find(
                    (option) => option.id === visualiser.transmission
                ),
                rearDoor: options.find(
                    (option) => option.id === visualiser.rearDoor
                ),
                paintwork: {
                    top: options.find(
                        (option) => option.id === visualiser.paintwork.top
                    ),
                    bottom: options.find(
                        (option) => option.id === visualiser.paintwork.bottom
                    ),
                },
                wheel: options.find((option) => option.id === visualiser.wheel),
                interior: options.find(
                    (option) => option.id === visualiser.interior
                ),
                oven_grill: this.$root.visualiser.oven_grill || undefined,
                trim: options.find((option) => option.id === visualiser.trim),
                worktop: options.find(
                    (option) => option.id === visualiser.worktop
                ),
                floor: options.find((option) => option.id === visualiser.floor),
                upholstery: options.find(
                    (option) => option.id === visualiser.upholstery
                ),
                oven: options.find((option) => option.id === visualiser.oven),
                gadgets: visualiser.gadgets.map((gadget) =>
                    options.find((option) => option.id === gadget)
                ),
                exterior: {
                    sidebars: options.find(
                        (option) => option.id === visualiser.exterior.sidebars
                    ),
                    rails: options.find(
                        (option) => option.id === visualiser.exterior.rails
                    ),
                    bike: options.find(
                        (option) => option.id === visualiser.exterior.bike
                    ),
                    towbar: options.find(
                        (option) => option.id === visualiser.exterior.towbar
                    ),
                    lip: options.find(
                        (option) => option.id === visualiser.exterior.lip
                    ),
                    blackGlossGrills: options.find(
                        (option) =>
                            option.id === visualiser.exterior.blackGlossGrills
                    ),
                    blackBadges: options.find(
                        (option) =>
                            option.id === visualiser.exterior.blackBadges
                    ),
                    spoiler: options.find(
                        (option) => option.id === visualiser.exterior.spoiler
                    ),
                    roof: options.find(
                        (option) => option.id === visualiser.exterior.roof
                    ),
                    solar: options.find(
                        (option) => option.id === visualiser.exterior.solar
                    ),
                },
                roof: {
                    colour: options.find(
                        (option) => option.id === visualiser.roof.colour
                    ),
                    tent: options.find(
                        (option) => option.id === visualiser.roof.tent
                    ),
                },
            };
        },
